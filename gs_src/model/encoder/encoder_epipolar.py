...
class EncoderEpipolar(Encoder[EncoderEpipolarCfg]):
    ...
    epipolar_transformer: Union[EpipolarTransformer, None]
    ...
    def forward(
        ...
        device = context["image"].device # (bs, 2,3,256,256)
        b, v, _, h, w = context["image"].shape

        # Encode the context images. No downsampling here.yes
        features = self.backbone(context)
        features = rearrange(features, "b v c h w -> b v h w c")
        features = self.backbone_projection(features)
        features = rearrange(features, "b v h w c -> b v c h w") # (b,v,c,h,w)
        ...
        skip = self.high_resolution_skip(skip) # still keeps the input resolution!
        ...
        ) # depth (bs, num_view, h*w, num_surface, num_gs)

        # Convert the features and depths into Gaussians.
        xy_ray, _ = sample_image_grid((h, w), device) # (256,256,2)
        xy_ray = rearrange(xy_ray, "h w xy -> (h w) () xy") # (256*256, 1, 2)
        gaussians = rearrange(
            self.to_gaussians(features),
            "... (srf c) -> ... srf c",
            srf=self.cfg.num_surfaces,
        ) # (bs, view, h*w, num_surface, 84)
        offset_xy = gaussians[..., :2].sigmoid() # why offset?
