...
model = dict(
    ...
    bbox_head=dict(
        type='VGGTDetHeadOneAngle_AngleIOU',
        ...
        matcher='one2more_angle',
        matcher_iou_thres=0.1,
        matcher_max_dynamic_samples=5,
        if_swap_length_and_width=True
        ),
    num_queries=256,
...
# data_root = '/project/visgroup/ycaobd/vggt_det_data/arkit/'
...
train_pipeline = [
    ...
    dict(type='ProjectPCtoFirstFrameAndNormArkit', coord_type='DEPTH'),
    # dict(type='NormBoxes'),
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        dataset=dict(
            type=dataset_type,
            data_root=data_root,
            ann_file='arkit_infos_train.pkl', #'scannet_infos_train_pts_10scenes.pkl', #'scannet_infos_train_pts.pkl',
...
val_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file='arkit_infos_val.pkl', #'scannet_infos_train_pts_10scenes.pkl', #'scannet_infos_val_pts.pkl', # 
...
optim_wrapper = dict(
    ...
    optimizer=dict(
        type='AdamW',
        lr=2.5e-4,  # 初始学习率 = 5e-4（峰值）
        weight_decay=1e-4
    ),
    clip_grad=dict(max_norm=35., norm_type=2)  # 3DETR 使用更严格的梯度裁剪，但这里用mvsdet的
)


param_scheduler = [
    # Warmup：1000 次迭代内从 1e-6 到 5e-4
    # dict(
    #     type='LinearLR',
    ...
    #     by_epoch=True,
    #     begin=0,
    #     end=_max_epoch  # 1000 次迭代 ≈ 1~2 Epoch（假设 Batch Size=32）
    # ),
    # Cosine 衰减到 1e-6
    dict(
...
default_hooks = dict(
    ...
    logger=dict(type='LoggerHook', interval=10)  # 每 10 个 iteration 打印一次
