...
class MVSDet(Base3DDetector):
    ...
    def __init__(
            ...
            N_samples: int = 64,
            N_rand: int = 2048,
            depth_supervise: bool = False, # scannet的训练是false
            use_nerf_mask: bool = True, # scannet的训练是false
            nerf_sample_view: int = 3,
            nerf_mode: str = 'volume',
            squeeze_scale: int = 4,
            rgb_supervision: bool = True, # scannet的训练是True
            nerf_density: bool = False,
            render_testing: bool = False,
            gs_cfg=None, # 较nerf新增
            vis_dir=None, # 较nerf新增
            visualize_bbox = False, # 较nerf新增
            topk=3,
            alpha_thres=None,
            sigma_w=0.5):
        super().__init__(
        ...
        self.depth_values = np.arange(self.near_far_range[0], self.near_far_range[1], self.depth_interval, # 把深度值等分
    ...
    def collect_proj(self, w2c, intr, neighbor_ids):# 计算每个相机及其相邻相机的 ​投影矩阵​（将3D世界点投影到2D图像坐标系）
        ''' 
    ...
    def sample_depth_prob(self, prob_volume, off_pred, topk=3):
        ...
        '''
        est_densities, est_idx = prob_volume.topk(k=topk, dim=1) # (num_src, k, h, w) # 用预测的概率当density，用est_idx来算depth
    ...
    def extract_feat(self,
        ...
        if len(img.shape) > 4:
            img = img.reshape([-1] + list(img.shape)[2:]) # 变成([40, 3, 240, 320])
            x = self.backbone(img)
            x = self.neck(x)[0] # (bs*40, f, 60, 80) neck即FPN
        ...
        features_2d = self.head_2d.forward(x[-1], batch_img_metas) \
            if self.head_2d is not None else None # ? what is head_2d   None, 所以 feature_2d也是None
        ...
        weight_gap_list = []
        src_rmse_list = []
        for feature, img_meta in zip(x, batch_img_metas): # TODO what inside img_metas ? 这里的feature是什么 feature.shape=([40, 256, 60, 80])
            angles = features_2d[
                0] if features_2d is not None and mode == 'test' else None # ? 为什么angle是 features_2d[0]
            projection = self._compute_projection(img_meta, stride, 
                                                  angles).to(x.device) # project lidar (3d) to img (2d) 
            points = get_points( # 搞一个voxel空间 points([3, 40, 40, 16])
            ...
            depth_values = torch.tensor(self.depth_values, device=x.device).unsqueeze(0).repeat(num_src, 1) # (bs, n_depth), n个depth
            ...
            est_depth, est_densities = self.sample_depth_prob(prob_volume, off_pred, topk=self.topk) # (num_src, num_gs, h, w)， 应该默认每个depth候选位置放一个高斯球

            ############idea1#############
            # est_depth, est_densities = self.sample_all_depth_prob(prob_volume, off_pred, topk=self.topk) # (num_src, num_gs, h, w)， 应该默认每个depth候选位置放一个高斯球
            ...
            depth_coding = self.compute_avg_depth(prob_volume, off_pred)[:,:height, :width].unsqueeze(1) # (num_src, 1, h, w) depth expectation，每个像素一个平均depth
            ...
            # 通过backprojection将特征图feat和深度信息est_depth转换为3D volume
            # feat volume

            # src_rmse是pred_depth和gt depth的均方误差(若没有gt depth, rmse = torch.tensor(1.))
            ...
            if ray_batch is not None: # rendering novel view
                ...
                depth_mode=None, # 'depth' to render depth. otherwise is None.原来是None，暂时改成depth看看出来的是多少
                scene=scene
                )
        ...
        # est_depth_crop 目前代码在任何情况下都是None
    ...
    def loss(self, batch_inputs_dict: dict, batch_data_samples: SampleList,
        ...
        if 'raydirs' in batch_inputs_dict.keys():
            ...
            ray_batchs['gt_rgb'] = batch_images # (bs, num_tgt, 3, 240, 320) 调试显示是[1, 2, 3, 120, 160]
...
def backproject_Weigh(features, points, projection, depth, voxel_size, prob, 
    ...
    with torch.no_grad():
        if gt_depth is not None: # (num_view, h, w)
            ...
            # with open("/HOME/yt_ust_danxu/yt_ust_danxu_4/HDD_POOL/yangcao/feize/MVSDet/work_dirs/mvsdet_res50_2x_low_res_1gpu_bs2/all_rmse.txt", "a") as f:  # 'a'表示追加模式
            #     for i in range(gt_depth.shape[0]):  # 遍历80个样本
            #     # 提取单张深度图
            #         single_gt = gt_depth[i]    # 形状 [59, 80]
            #         single_pred = pred_depth[i]  # 形状 [59, 80]
                    
            #         # 生成有效掩码
            #         # valid_mask = single_gt > 0  # 形状 [59, 80]
            #         eval_min_depth=0.5
            #         eval_max_depth=10
            #         valid_mask = (single_gt > eval_min_depth) & (single_gt < eval_max_depth)
                    
            #         # 计算单样本RMSE
            #         valid_gt = single_gt[valid_mask]    # 形状 [M]
            #         valid_pred = single_pred[valid_mask] # 形状 [M]
            #         if len(valid_gt) == 0:
            #             continue  # 跳过全无效样本
            #         rmse = torch.sqrt(torch.mean((valid_pred - valid_gt)**2))
                    
            #         # 写入文件（每行一个RMSE）
            #         f.write(f"{rmse.item():.8f}\n")  # 直接使用已打开的文件句柄
            ##################################
    # ----------------- end debug -------------------
