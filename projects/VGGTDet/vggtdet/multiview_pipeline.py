...
class ProjectPCtoFirstFrameAndNormArkit(BaseTransform):
    ...
    def __init__(self, coord_type, if_visualize_boxes=False):
        self.coord_type = coord_type
        self.if_visualize_boxes = if_visualize_boxes
    ...
    def transform(self, results: dict) -> dict:
        ...
        points_class = get_points_type(self.coord_type)
        if not self.if_visualize_boxes:
            results['points'] = points_class(pc_cam_with_rgb, points_dim=points.points_dim, attribute_dims=points.attribute_dims)
        else:
            results['points_first_axis'] = points_class(pc_cam_with_rgb, points_dim=points.points_dim, attribute_dims=points.attribute_dims)
