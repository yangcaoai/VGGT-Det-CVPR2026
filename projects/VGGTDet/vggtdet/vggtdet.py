...
class VGGTDet(Base3DDetector):
    def __init__(
            ...
            lambda_dist=1.0,
            if_task_query=False,
            if_project_first_frame_points_back=False
            ):
        ...
        if os.path.exists("/data/yang/cache/huggingface/hub/models--facebook--VGGT-1B/"):
            self.vggt_encoder = VGGT.from_pretrained("/data/yang/cache/huggingface/hub/models--facebook--VGGT-1B/").to(device)
        else:
        ...
        self.if_project_first_frame_points_back = if_project_first_frame_points_back
        # assert 
    ...
    def get_box_features(self, vggt_token_list, ps_idx, batch_inputs_dict, images, images_patch_attn):
        ...
        if self.if_use_gt_query:
            ...
            # points_xyz = torch.stack(batch_inputs_dict['points'], dim=0)[:, :, :3].contiguous()
            # gt_points = batch_inputs_dict['points'][0]
            # write_ply_rgb(gt_points.cpu().numpy(), 'vis_pc_arkit/1_gt_pc_rgb.ply')
            # write_ply(points_xyz[0], 'vis_pc_arkit/1_gt_pc.ply')
            pred_pc, atten_weights = self.pred_pc_from_vggt(vggt_token_list, ps_idx, images, batch_inputs_dict, images_patch_attn)
            if self.if_project_first_frame_points_back:

                def project_first_frame_points_back(pred_pc, pose_matrix, align_matrix):
                    pose_matrix = torch.stack(pose_matrix, dim=0).to(pred_pc.device, dtype=pred_pc.dtype)  # [16, 4, 4]
                    align_matrix = torch.stack(align_matrix, dim=0).to(pred_pc.device, dtype=pred_pc.dtype)
                    transform_matrix = torch.matmul(align_matrix.float(), pose_matrix.float())
                    # Convert points to homogeneous coordinates [bs, num_points, 4]
                    num_points = pred_pc.shape[1]
                    homogeneous_points = torch.cat([pred_pc, torch.ones_like(pred_pc[..., :1])], dim=-1)
                    
                    # Apply transformation: [bs, 4, 4] @ [bs, 4, num_points] -> [bs, 4, num_points]
                    transformed_points = torch.matmul(
                        transform_matrix, 
                        homogeneous_points.transpose(1, 2)
                    ).transpose(1, 2)  # Back to [bs, num_points, 4]
                    
                    # Convert back to 3D coordinates
                    transformed_pc = transformed_points[..., :3] / transformed_points[..., 3:4]  # Perspective division
                    # print(transformed_pc.shape)
                    # write_ply(transformed_pc[0], 'vis_pc_arkit/1_transform_pc.ply')
                    
                    return transformed_pc
                
                pred_pc = project_first_frame_points_back(pred_pc, batch_inputs_dict['pose_matrix'], batch_inputs_dict['axis_align_matrix']) # [1, 100000, 3]   len(batch_inputs_dict['pose_matrix'])=bs
            if self.if_use_atten_fps:
    ...
    def loss(self, batch_inputs_dict: dict, batch_data_samples: SampleList,
        ...
        # b = batch_inputs_dict['imgs'].cpu()

        if self.if_mix_precision:
            with torch.cuda.amp.autocast(dtype=vggt_dtype):
                box_features = self.get_box_features(vggt_token_list, ps_idx, batch_inputs_dict, img, images_patch_attn) # time < 1losses = self.bbox_head.loss(box_features, batch_data_samples, batch_inputs_dict, **kwargs) 
        else: 
    ...
    def attention_guided_prob_fps(self,
        ...
        assert points.dim() == 3, "the input should be 3D tensor with shape (B, N, 3)"
        assert attention_weights.shape == points.shape[:2], "the attention weights shape does not match the points shape"
