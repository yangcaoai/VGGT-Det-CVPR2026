...
from projects.VGGTDet.detr3_models.helpers import (ACTIVATION_DICT, NORM_DICT, WEIGHT_INIT_DICT,
                            get_clones)
...
class TransformerDecoder_Multilevel(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        for idx_layer, layer in enumerate(self.layers):
            if if_task_query:
                # 1. 提取 task query (最后一个query)
                task_query = output[-1:]  # [1, batch_size, feat_dim] ?
                
                # 2. 通过MLP生成4个权重（对应4层memory）
                task_query = task_query.permute(1, 2, 0)  #[bs, feat_dim, 1]
                task_weights = self.task_head(task_query)  
                task_weights = torch.softmax(task_weights, dim=-2)  # 权重归一化
                
                # 3. 将4层memory加权平均
                # 展开权重：[1, batch_size, 4] -> [batch_size, 4, 1, 1]
                weights = task_weights.permute(1, 2, 0).unsqueeze(-1)  # 用于广播
                
                # 将4层memory堆叠：[4, H*W, batch_size, feat_dim]
                stacked_memory = torch.stack(memory, dim=0)
                
                # 加权平均：[batch_size, 4, 1, 1] * [4, H*W, batch_size, feat_dim] -> sum(0)
