...
def get_3d_box_batch_depth_tensor(box_size, angle, center):
    ...
    # --------------- 核心修改部分 ---------------
    # 1. 修改旋转轴为Z轴
    R = rotz_tensor_batch(angle)  # 替换原版的roty
    
    # 2. 调整坐标轴分配
    l = torch.unsqueeze(box_size[..., 0], -1)  # X轴长度
    h = torch.unsqueeze(box_size[..., 1], -1)  # Y轴高度（向下为正）
    w = torch.unsqueeze(box_size[..., 2], -1)  # Z轴深度（向前为正）
    ...
    # Y轴坐标（上下方向取反，适配IOU函数对负Y为上的假设）
    corners_3d[..., 1] = torch.cat(
        [-h/2, -h/2, -h/2, -h/2, h/2, h/2, h/2, h/2], dim=-1  # 顶部Y为负值
    )
    ...
    # 应用旋转（绕Z轴）
    corners_3d = torch.matmul(corners_3d, R.permute(0, 2, 1))  # 调整矩阵乘法顺序
