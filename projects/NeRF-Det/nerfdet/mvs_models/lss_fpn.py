...
class DepthNet(nn.Module):
    ...
    # @autocast(False)
    def forward(self, x, intrinsic, w2c):
        '''
        x: src feat. (num_src, f, h, w)
        intrinsic: (bs, num_cam, 4, 4)
        w2c: (bs, num_cam, 4, 4)
        bs = 1
        return:
            mu: (b*n_cam, num_range, h, w)
            sigma: (b*n_cam, num_range, h, w)
            range score: (b*n_cam, num_range, h, w)
        '''
        B, _, H, W = x.shape # (batch_size * num_cams, f,h,w)
        # intrins = mats_dict['intrin_mats'][:, 0:1, ..., :3, :3] # (B, num_sweeps, num_cameras, 3, 3). get cam intrsinsic for every cam in this sweep
        intrins = intrinsic[:,:,:3,:3] # (bs, num_cam, 3, 3)
        batch_size = intrins.shape[0]
        assert batch_size == 1 
        num_cams = intrins.shape[1]
        sensor2ego = w2c[:,:,:3,:] # (B, num_src, 3, 4)
        ...
        mlp_input = torch.cat(
            ...
            -1,
        ) # (b, n_src, n_cam_params)
        
        mlp_input = self.bn(mlp_input.reshape(-1, mlp_input.shape[-1]))
        assert x.shape[0] == mlp_input.shape[0]
        x = self.reduce_conv(x) # (b*n_cam, f', h,w)
        
        depth_se = self.depth_mlp(mlp_input)[..., None, None] # (b*n_cam, f',1,1)
        depth_feat = self.depth_se(x, depth_se) # (b*n_cam, f',h,w)
        # depth_feat = self.depth_feat_conv(depth_feat) # TODO: removed for simplicity.
        # print('depth_feat: {}'.format(depth_feat.shape))
        mu_sigma_score = self.mu_sigma_range_net(depth_feat) # (b*n_cam, num_range*3, h, w)
        # print('mu_sigma_score: {}'.format(mu_sigma_score.shape))
        mu = mu_sigma_score[:, 0:self.num_ranges, ...] # (b*n_cam, num_range, h, w)
        sigma = mu_sigma_score[:, self.num_ranges:2 * self.num_ranges, ...] # (b*n_cam, num_range, h, w)
        range_score = mu_sigma_score[:,
                                     2 * self.num_ranges:3 * self.num_ranges,
                                     ...] # (b*n_cam, num_range, h, w)
        sigma = F.elu(sigma) + 1.0 + 1e-10
        return mu, sigma, range_score
...
class LSSFPN(nn.Module):
    ...
    def depth_sampling(self):
        ...
        idx_list = np.arange(0, self.num_samples + 1) # num_sample=3
    ...
    def _generate_cost_volume(
        ...
        for idx in range(num_sweeps):
            ...
            warped_stereo_fea = self.homo_warping(
                stereo_feats_all_sweeps[idx], # (bs*n_cam, num_channels, height, width) src feat
            ...
            ) # (bs*n_cams, f, n_depth, h, w)
            warped_stereo_fea = warped_stereo_fea.reshape(
                batch_size, self.num_groups, num_channels // self.num_groups,
                self.num_samples, height, width) # (bs*n_cams, n_group, f/n_group, n_depth, h, w)
            ref_stereo_feat = stereo_feats_all_sweeps[sweep_index].reshape(
                batch_size, self.num_groups, num_channels // self.num_groups,
                height, width) # (bs*n_cams, n_group, f/n_group, h, w)
            feat_cost = torch.mean(
                (ref_stereo_feat.unsqueeze(3) * warped_stereo_fea), axis=2) # (bs*n_cams, n_group, n_depth, h, w)
            depth_score = self.similarity_net(feat_cost).squeeze(1) # (bs*n_cams, n_depth, h, w). 3d convblock
            depth_score_all_sweeps.append(depth_score)
        return torch.stack(depth_score_all_sweeps).mean(0) # (bs*n_cams, n_depth, h, w)
    ...
    def _forward_stereo(
        ...
        d_coords = torch.arange(*self.d_bound,
                                dtype=torch.float,
                                device=device).reshape(1, -1, 1, 1) # (1, n_depth, 1, 1)
        d_coords = d_coords.repeat(batch_size_with_cams, 1, feat_height,
                                   feat_width) # (b*n_cam, n_depth, h, w)
        stereo_depth = stereo_feats_all_sweeps[0].new_zeros(
            batch_size_with_cams, self.depth_channels, feat_height, feat_width) # (b*n_cam, n_depth, h, w)
        ...
        range_score = range_score_all_sweeps[sweep_index].softmax(1) # (b*n_cam, n_range, h, w)
        for range_idx in range(self.num_ranges):
            ...
            ] # [(b*n_cam, 1, h, w), (b*n_cam, 1, h, w),  ...] len=n_sweep
            ...
            batch_size_with_cams, _, feat_height, feat_width =\
                stereo_feats_all_sweeps[0].shape
            mu = mu_all_sweeps_single_range[sweep_index] # (b*n_cam, 1, h, w) mu for current sweep current range
            sigma = sigma_all_sweeps_single_range[sweep_index] # (b*n_cam, 1, h, w)
            for _ in range(self.em_iteration):
                depth_sample = torch.cat([mu + sigma * k for k in self.k_list],
                                         1) # (b*n_cam, n_depth, h, w) n_depth=3
                depth_sample_frustum = self.create_depth_sample_frustum(
                    depth_sample, self.stereo_downsample_factor) # b*n_cam x D x H x W x 4
                ...
                mu = (depth_sample * mu_score).sum(1, keepdim=True) # (bs*n_cams, 1, h, w)
                del depth_sample
            ...
            mu_repeated = mu.repeat(1, range_length, 1, 1) # (n*n_cam, num_depth_bins, h, w)
            eps = 1e-6
            depth_score_single_range = (-1 / 2 * (
                ...
                torch.sqrt(sigma))**2) # eqn. 4 sample many depth candidates
            depth_score_single_range = depth_score_single_range.exp() # (n*n_cam, num_depth_bins, h, w)
            score_all_ranges.append(mu_score.sum(1).unsqueeze(1)) # doesn't have meaning
            ...
            if sweep_index > 0:
                ...
                context_all_sweeps.append(
                    self.context_downsample_net(
                        context.reshape(batch_size * num_cams,
                                        *context.shape[1:])))
            mu_all_sweeps.append(mu) # (b*n_cams, num_range, h, w)
            sigma_all_sweeps.append(sigma) # (b*n_cams, num_range, h, w)
            mono_depth_all_sweeps.append(mono_depth)
            range_score_all_sweeps.append(range_score) # (b*n_cams, num_range, h, w)
        depth_score_all_sweeps = list()
        ...
        for ref_idx in range(num_sweeps):
            ...
            for src_idx in range(num_sweeps):
                ref2keysensor_mats = mats_dict[
                    'sensor2sensor_mats'][:, ref_idx, ...].inverse() # transformation from sweep frame to key frame
